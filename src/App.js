...
import { ApiHost } from "./Components/Common/Config";
...
class App extends React.Component {
  ...
  constructor(props) {
    ...
    if (localStorage.getItem("LogToken") + "".length !== 0) {
      axios({
        method: "get",
        url: ApiHost + "/api/TokenVerification",
        params: {
          token: localStorage.getItem("LogToken"),
        ...
        .then(res => {
          ...
          if (res.data.error + "" !== "none") {
            ...
            details.AvatarPath =
              ApiHost + "/images/Avatars/" + details.AvatarPath;
  ...
  render() {
    ...
    if (this.state.loged) {
      if (this.state.user.UserType === "etud") return <Student />;
      else if (this.state.user.UserType === "prof") return <Prof />;
    } else return <Landing />;
  }
}
